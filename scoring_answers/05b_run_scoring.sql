...
-- ----------------------------------------------------------------------------
-- Stage 1   - score every (response x ideologue) pair into SCORE_AUDIT.
-- Stage 1.5 - retry any rows whose score did not parse (top_k bumped to 12).
-- Stage 2   - pivot SCORE_AUDIT into the 8 _BOT columns on RESPONSES_ENRICHED.
--
-- Expect ~25-45 min on the X-SMALL MODELDNA_WH warehouse for 200 x 8 = 1600
-- Cortex Complete calls. Cortex Complete is rate-limited per account, not
-- per warehouse, so larger warehouses will not speed this up linearly.
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- Stage 1: score everything. CROSS JOIN responses to ideologues, call the
-- UDF once per pair, insert the structured result into SCORE_AUDIT.
...
-- ----------------------------------------------------------------------------
-- Quick post-run summary. Run 06_calibration.sql for the full sanity suite.
-- ----------------------------------------------------------------------------
